...
| `leaderboard.js` | `Leaderboard` | `paintList({listSelector, showSince?, sinceFormatter?, highlight?})` → an `<ol>` (conventionally `#lb-list`); `paintHiScoreRow({valueSelector, initialsSelector, rowSelector})` | Cloud (`/api/leaderboard`) + localStorage mirror. `init({game, max})` — `game` must be in the worker allowlist (`infra/leaderboard-worker/src/worker.ts`). `submit()` inserts optimistically (instant local feedback; cloud reconciles). `showSince` adds the "since" column (+`.has-since` grid); `highlight: {score, initials}` flags that row `.is-you`. Mirror key: `oyster-arcade-leaderboard-<game>`. |
...
- **`leaderboard.css`** — `.leaderboard` › `.lb-title` / `.lb-list` (`li`) / `.lb-rank` / `.lb-initials` / `.lb-score` / `.lb-empty`. Optional 4th column `.lb-since` (enabled by `.has-since`, which `paintList` adds when `showSince`); `.lb-list li.is-you` highlights the player's just-entered row (flash → pulse). Tune the heading via `--lb-title-size` (set on `.leaderboard`; inherits to `.lb-title`).
